...
export default function MartingalePage() {
  ...
  const createMartingale = async () => {
    if (!martingaleName || !targetProfit) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Erreur lors de la création de la martingale: ' + (error.error || error.message || 'Erreur inconnue'))
      }
    } catch (error) {
  ...
  const addPalier = async () => {
    if (!activeMartingale || pronos.some(p => !p.odds)) return

    try {
      const response = await fetch('/api/martingales/paliers', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // Abandonner la martingale active
  const abandonMartingale = async () => {
    if (!activeMartingale || !isAuthenticated) return
    
    if (!confirm('Êtes-vous sûr de vouloir abandonner cette martingale ? Elle sera archivée comme perdue.')) {
      return
    }

    try {
      const response = await fetch('/api/martingales/abandon', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const renderGlobalEvolutionChart = () => {
    ...
    sortedMartingales.forEach(martingale => {
      cumulativeProfit += parseFloat(martingale.net_profit || 0)
      chartData.push({ 
        value: cumulativeProfit, 
        martingale: martingale,
        profit: parseFloat(martingale.net_profit || 0)
      })
    })
              ...
              return (
                <g key={index}>
                  <circle
                    ...
                    cy={y}
                    r="4"
                    fill={data.martingale.status === 'completed' ? '#10b981' : '#ef4444'}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col sm:flex-row justify-between items-center py-4 sm:py-6 space-y-2 sm:space-y-0">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 text-center flex-1">
                Nos Martingales - Paris Sportifs
              </h1>
              <div className="flex items-center space-x-2 sm:space-x-4">
                {/* BOUTON COMMENT ÇA MARCHE */}
                <Link 
                  href="/paris-sportifs/martingale/comment-ca-marche" 
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="border-t pt-3">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Profit net total:</span>
                      <span className={`font-medium ${
                        archivedMartingales.reduce((sum, m) => sum + parseFloat(m.net_profit || 0), 0) >= 0 
                          ? 'text-green-600' 
                          : 'text-red-600'
                      }`}>
                        {archivedMartingales.reduce((sum, m) => sum + parseFloat(m.net_profit || 0), 0).toFixed(2)}€
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {archivedMartingales.length > 0 && (
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="bg-gray-50 rounded p-2 text-center">
                      <span className="text-gray-600 block">Nb moyen paliers</span>
                      <p className="font-bold text-base sm:text-lg">
                        {(archivedMartingales.reduce((sum, m) => sum + (m.paliers?.length || 0), 0) / archivedMartingales.length).toFixed(1)}
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  {archivedMartingales.slice(0, 5).map(m => (
                    ...
                    >
                      <div className="flex justify-between">
                        ...
                        <span className={parseFloat(m.net_profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'}>
                          {parseFloat(m.net_profit || 0) >= 0 ? '+' : ''}{parseFloat(m.net_profit || 0).toFixed(2)}€
                        </span>
                      </div>
                      <div className="text-gray-500 text-xs">
                        {new Date(m.end_date).toLocaleDateString()} - {m.paliers?.length || 0} paliers
                      </div>
                    </div>
                  ))}
                </div>
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start">
                          ...
                          <div className="text-left sm:text-right">
                            <span className={`inline-block px-2 py-1 rounded text-xs sm:text-sm ${
                              ...
                              {m.status === 'completed' ? 'Réussie' : 'Abandonnée'}
                            </span>
                            <p className={`mt-2 font-bold ${parseFloat(m.net_profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {parseFloat(m.net_profit || 0) >= 0 ? '+' : ''}{parseFloat(m.net_profit || 0).toFixed(2)}€
                            </p>
                            {isAuthenticated && (
                              <button
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                        ...
                        <span className={`inline-block px-2 py-1 rounded text-xs sm:text-sm ${
                          selectedArchivedMartingale.status === 'completed' 
                            ? 'bg-green-100 text-green-800' 
                            : 'bg-red-100 text-red-800'
                        }`}>
                          {selectedArchivedMartingale.status === 'completed' ? 'Réussie' : 'Abandonnée'}
                        </span>
                      </div>
                  ...
                  <div className="bg-gray-50 rounded p-4 mb-6">
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Objectif profit</span>
                      ...
                      <div>
                        ...
                        <p className="font-bold text-sm sm:text-base">{parseFloat(selectedArchivedMartingale.total_invested || 0).toFixed(2)}€</p>
                      </div>
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Nombre de paliers</span>
                        <p className="font-bold text-sm sm:text-base">{selectedArchivedMartingale.paliers?.length || 0}</p>
                      </div>
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Profit net final</span>
                        <p className={`font-bold text-sm sm:text-base ${
                          parseFloat(selectedArchivedMartingale.net_profit || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                        }`}>
                          {parseFloat(selectedArchivedMartingale.net_profit || 0) >= 0 ? '+' : ''}{parseFloat(selectedArchivedMartingale.net_profit || 0).toFixed(2)}€
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="bg-red-50 rounded p-4 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:gap-4">
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Objectif profit</span>
                        <p className="font-bold text-sm sm:text-base">+{parseFloat(activeMartingale.target_profit).toFixed(2)}€</p>
                      </div>
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Total investi</span>
                        <p className="font-bold text-sm sm:text-base text-red-600">{parseFloat(activeMartingale.total_invested || 0).toFixed(2)}€</p>
                      </div>
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Pertes cumulées</span>
                        <p className="font-bold text-sm sm:text-base text-red-600">-{parseFloat(activeMartingale.current_losses || 0).toFixed(2)}€</p>
                      </div>
                      <div>
                        <span className="text-xs sm:text-sm text-gray-600">Mise nécessaire</span>
                        <p className="font-bold text-sm sm:text-base text-amber-600">
                          ~{Math.ceil((parseFloat(activeMartingale.current_losses || 0) + parseFloat(activeMartingale.target_profit)) / 0.7)}€
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 text-xs text-gray-600">
                      <strong>Note :</strong> La mise nécessaire est estimée pour une cote moyenne de 1.70
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {activeMartingale.paliers && [...activeMartingale.paliers].reverse().map(palier => (
                      <div key={palier.id} className={`border rounded-lg p-4 ${
                        palier.status === 'pending' ? 'border-red-400 bg-red-50' : ''
                      }`}>
                        <div className="flex flex-col sm:flex-row justify-between items-start mb-2">
                          <h3 className="font-medium text-sm sm:text-base mb-2 sm:mb-0">
                            Palier {palier.number} - {getPalierType(palier)}
                            ...
                            {palier.status === 'pending' && (
                              <span className="ml-2 text-xs sm:text-sm text-red-600 font-normal block sm:inline mt-1 sm:mt-0">⏳ En attente de validation</span>
                            )}
                          </h3>
                          <div className="flex items-center space-x-2">
            ...
            <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="bg-amber-50 border border-amber-300 rounded p-3 mb-4">
                <p className="text-sm text-amber-800">
                  <strong>⚠️ Attention :</strong> Les martingales sont à très haut risque. 
                  Assurez-vous d'avoir une bankroll suffisante.
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Martingale Tennis"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Le profit net visé après récupération de toutes les pertes
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  ...
                  disabled={!martingaleName || !targetProfit}
                >
                  Créer
                </button>
              </div>
        ...
        {showAddPalier && isAuthenticated && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto p-4">
            <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-2xl my-4 max-h-[90vh] overflow-y-auto">
              ...
              {activeMartingale && (
                <div className="bg-red-50 rounded p-3 mb-4">
                  <p className="text-sm text-red-800">
                    <strong>Mise calculée :</strong> Pour récupérer {parseFloat(activeMartingale.current_losses || 0).toFixed(2)}€ 
                    et gagner {parseFloat(activeMartingale.target_profit).toFixed(2)}€, 
                    la mise dépendra de la cote totale sélectionnée.
                  </p>
                </div>
              )}
              ...
              {pronos.some(p => p.odds) && activeMartingale && (
                <div className="mt-4 p-3 bg-gray-50 rounded">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <span className="text-sm text-gray-600">Mise nécessaire:</span>
                    <span className="font-bold text-lg text-red-600">
                      {((parseFloat(activeMartingale.current_losses || 0) + parseFloat(activeMartingale.target_profit)) / 
                        (pronos.reduce((acc, p) => acc * (parseFloat(p.odds) || 1), 1) - 1)).toFixed(2)}€
                    </span>
                  </div>
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <span className="font-bold text-lg text-green-600">
                      {(((parseFloat(activeMartingale.current_losses || 0) + parseFloat(activeMartingale.target_profit)) / 
                        (pronos.reduce((acc, p) => acc * (parseFloat(p.odds) || 1), 1) - 1)) * 
                        pronos.reduce((acc, p) => acc * (parseFloat(p.odds) || 1), 1)).toFixed(2)}€
                    </span>
                  </div>
                  <div className="flex justify-between items-center mt-2">
                    <span className="text-sm text-gray-600">Profit net si gagné:</span>
                    <span className="font-bold text-lg text-green-600">
                      +{parseFloat(activeMartingale.target_profit).toFixed(2)}€
                    </span>
                  </div>
                </div>
              )}
